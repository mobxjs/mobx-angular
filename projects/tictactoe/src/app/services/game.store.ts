...
export class GameStore {
  ...
  @computed get moves(): number {
    return (
      this.board[0].filter(cell => cell).length +
  ...
  @computed get winner(): string {
    ...
    // rows:
    if (this._rowHasWinner(0)) {
      return this.board[0][0];
    }
    if (this._rowHasWinner(1)) {
      return this.board[1][0];
    }
    if (this._rowHasWinner(2)) {
      return this.board[2][0];
    }

    // columns:
    if (this._columnHasWinner(0)) {
      return this.board[0][0];
    }
    if (this._columnHasWinner(1)) {
      return this.board[0][1];
    }
    if (this._columnHasWinner(2)) {
